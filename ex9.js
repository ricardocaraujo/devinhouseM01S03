...
listaCamarote.forEach(convidado => {
    camarote.innerHTML += `<li>${montaNome(convidado)} ${marcarOpenBar(convidado)}</li>`;
});

listaArquibancada.forEach(convidado => {
    arquibancada.innerHTML += `<li>${montaNome(convidado)} ${marcarOpenBar(convidado)}</li>`;
})

listaPista.forEach(convidado => {
    pista.innerHTML += `<li>${montaNome(convidado)} ${marcarOpenBar(convidado)}</li>`;
})